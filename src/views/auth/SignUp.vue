<template>
  <div class="auth-page">
    <div class="auth-page__header">
      <h4 class="auth-page__title">Sign Up</h4>
    ...
    <div class="auth-demo-wrap">
      <div class="auth-card-demo">
        ...
        <h2 class="acd-title">Create Account 🚀</h2>
        <p class="acd-sub">Get started with AL-AHMODANY Admin.</p>

        <div class="acd-field" :class="{ error: errors.name }">
          <label class="acd-label">Full Name</label>
          <div class="acd-input-wrap">
            <i class="bx bx-user acd-input-icon"></i>
            <input v-model="form.name" type="text" placeholder="John Doe" class="acd-input" />
          </div>
          <span v-if="errors.name" class="acd-error">{{ errors.name }}</span>
        </div>
        ...
        <div class="acd-field" :class="{ error: errors.email }">
          ...
          <div class="acd-input-wrap">
            ...
            <input v-model="form.email" type="email" placeholder="john@example.com" class="acd-input" />
          </div>
        ...
        <div class="acd-field" :class="{ error: errors.confirm }">
          <label class="acd-label">Confirm Password</label>
          <div class="acd-input-wrap">
            <i class="bx bx-lock-alt acd-input-icon"></i>
            <input
            ...
            />
            <button class="acd-eye" type="button" @click="showConfirm = !showConfirm">
              <i :class="showConfirm ? 'bx bx-hide' : 'bx bx-show'"></i>
            </button>
          </div>
          <span v-if="errors.confirm" class="acd-error">{{ errors.confirm }}</span>
        </div>
...
function submit() {
  ...
  else if (password.length < 6)  errors.value.password = "Min. 6 characters.";
  if (!confirm)                  errors.value.confirm  = "Please confirm password.";
  else if (confirm !== password) errors.value.confirm  = "Passwords do not match.";
  ...
  setTimeout(() => {
    ...
    const session = {
      ...
      avatar: "", firstName: newUser.name.split(" ")[0], loginTime: Date.now(),
    };
...
.acd-logo { display: flex; align-items: center; gap: 8px; margin-bottom: 24px; }
