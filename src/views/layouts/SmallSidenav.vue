<template>
  <div class="layout-page">
    <div class="layout-page__header">
      <h4 class="layout-page__title">Small Sidenav</h4>
      <nav class="layout-page__breadcrumb">
        <span>Layouts</span><span class="sep">›</span><span class="active">Small Sidenav</span>
      </nav>
    </div>

    <div class="layout-info-card">
      <div class="layout-info-card__icon" style="background:linear-gradient(135deg,#2ecc71,#27ae60)">
        <i class="bx bx-sidebar"></i>
      </div>
      <div class="layout-info-card__body">
        <h5>Small Sidebar (Hover Expand)</h5>
        <p>Icon-only sidebar that expands to full width on hover. Maximizes content space while keeping navigation accessible.</p>
        <button class="layout-btn" :class="{ applied }" @click="apply">
          <i :class="applied ? 'bx bx-check' : 'bx bx-palette'"></i>
          {{ applied ? 'Applied!' : 'Apply Small Sidenav' }}
        </button>
      </div>
    </div>

    <div class="layout-preview">
      <div class="preview-sidebar preview-sidebar--small">
        <div class="preview-logo-dot"></div>
        <div v-for="i in 7" :key="i" class="preview-nav-dot" :class="{ active: i === 1 }"></div>
      </div>
      <div class="preview-content">
        <div class="preview-topbar"></div>
        <div class="preview-body">
          <div v-for="i in 4" :key="i" class="preview-card"></div>
        </div>
      </div>
    </div>
  </div>
...
function apply() {
  applyLayoutSettings({ sidebarSize: "sm-hover" });
  applied.value = true;
...
.layout-btn { display: inline-flex; align-items: center; gap: 6px; padding: 8px 20px; background: #2ecc71; color: #fff; border: none; border-radius: 6px; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s; }
...
.preview-sidebar--small { width: 42px; background: #313a46; display: flex; flex-direction: column; gap: 8px; padding: 14px 8px; flex-shrink: 0; align-items: center; }
.preview-logo-dot { width: 18px; height: 18px; background: rgba(255,255,255,0.2); border-radius: 4px; margin-bottom: 10px; }
.preview-nav-dot { width: 18px; height: 18px; background: rgba(255,255,255,0.1); border-radius: 4px; }
.preview-nav-dot.active { background: #fd7e14; }
.preview-content { flex: 1; display: flex; flex-direction: column; background: var(--bg-body); }
.preview-topbar { height: 38px; background: var(--card-bg); border-bottom: 1px solid var(--card-border); }
.preview-body { flex: 1; display: grid; grid-template-columns: 1fr 1fr; gap: 10px; padding: 12px; }
