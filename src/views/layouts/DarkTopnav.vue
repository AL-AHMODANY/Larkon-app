<template>
  <div class="layout-page">
    <div class="layout-page__header">
      <h4 class="layout-page__title">Dark Topnav</h4>
      <nav class="layout-page__breadcrumb">
        <span>Layouts</span><span class="sep">›</span><span class="active">Dark Topnav</span>
      </nav>
    </div>

    <div class="layout-info-card">
      <div class="layout-info-card__icon" style="background:linear-gradient(135deg,#5b73e8,#4a5fd4)">
        <i class="bx bx-layout"></i>
      </div>
      <div class="layout-info-card__body">
        <h5>Dark Topbar Layout</h5>
        <p>A dark-themed topbar with a light or dark sidebar. Great for a professional, modern admin look.</p>
        <button class="layout-btn" :class="{ applied }" @click="apply">
          <i :class="applied ? 'bx bx-check' : 'bx bx-palette'"></i>
          {{ applied ? 'Applied!' : 'Apply Dark Topnav' }}
        </button>
      </div>
    </div>

    <div class="layout-preview">
      <div class="preview-sidebar preview-sidebar--dark">
        <div class="preview-logo"></div>
        <div v-for="i in 7" :key="i" class="preview-nav-item" :class="{ active: i === 1 }"></div>
      </div>
      <div class="preview-content">
        <div class="preview-topbar preview-topbar--dark"></div>
...
function apply() {
  applyLayoutSettings({ topbarColor: "dark" });
  applied.value = true;
...
.layout-btn { display: inline-flex; align-items: center; gap: 6px; padding: 8px 20px; background: #5b73e8; color: #fff; border: none; border-radius: 6px; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s; }
...
.preview-sidebar--dark { background: #313a46; }
.preview-logo { height: 22px; background: rgba(255,255,255,0.18); border-radius: 4px; margin-bottom: 10px; }
.preview-nav-item { height: 9px; background: rgba(255,255,255,0.1); border-radius: 3px; }
...
.preview-topbar--dark { background: #313a46 !important; border-bottom-color: #404954 !important; }
